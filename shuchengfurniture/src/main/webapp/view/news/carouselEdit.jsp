	...
	<section>
		...
		<div id="main-content" class="main-content">
			...
			<div class="wrapper">
				<div class="row">
					<div class="col-sm-12">
						<form class="form-horizontal adminex-form" enctype="multipart/form-data" method="post" action="../news/saveEditNews?ac=<%=ac%>&lbType=${type}"> 
		                    <div class="form-group">
		                        <label class="col-sm-1  control-label">标题:</label>
		                        <div class="col-sm-11">
		                          <input type="text" style="width: 81%" name="title" class="form-control" value="${news.title }" placeholder="请输入标题"/>
		                       	   <input type="hidden" name="id" id="id" value="${news.id}" class="form-control" /> 
		                        </div> 
		                    </div>
		                    <div class="form-group">
		                        <label class="col-sm-1  control-label">作者:</label>
		                        <div class="col-sm-4"> 
		                         <input type="text"  name="author"  class="form-control" value="${news.author }" placeholder="请输入作者"/>
		                        </div> 
		                         <label class="col-sm-1  control-label">序号:</label>
		                        <div class="col-sm-4"> 
		                         <input type="text"  name="orderId"  class="form-control" value="${news.orderId }" placeholder="请输入序号"/>
		                        </div> 
		                    </div>
		                    ...
		                    <div class="form-group"> 
		                        ...
		                        <div class="col-sm-2">
		                           <c:if test="${not empty news.imgUrl}"><img src="<%=basePath%>${news.imgUrl}" style="width: 35px;height:35px;"/></c:if>
		                        </div>
		                        <label class="col-sm-1">是否上线: </label>
		                        <label class="col-sm-1">
		                               	是:<input type="radio"  class="radio-inline" name="isOnline" id="isOnline1"  value="1" <c:if test="${news.isOnline==1}">checked</c:if>/>
		                        </label>         
		                        <label class="col-sm-2">          
		                                                                                                否:<input type="radio"  class="radio-inline" name="isOnline" id="isOnline2"  value="0" <c:if test="${news.isOnline==0}">checked</c:if>/>
		                        </label>
