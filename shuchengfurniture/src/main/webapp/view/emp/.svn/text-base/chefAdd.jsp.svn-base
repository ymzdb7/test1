...
<body class="sticky-header">
	...
	<section>
		...
		<div id="main-content" class="main-content">
			...
			<div class="wrapper">
				<div class="row">
					<div class="col-sm-12">
						<form class="form-horizontal adminex-form" id="add_frm" action="chefSave.do"
			namespace="/chef" method="post" enctype="multipart/form-data">
		                    ...
		                    <div class="form-group">
		                        ...
		                        <div class="col-sm-8">
		                            <table id="tab1">
										<tr>
										 <td>
											<div class="col-sm-5">
					                            <input type="text" class="form-control tooltips" name="hc_date_start" value="${hc_date_starts }" autocomplete="off"   onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})" readonly="readonly" />
					                        </div>
					                        <label class="col-sm-1  control-label">~</label>
					                        <div class="col-sm-5">
					                            <input type="text" class="form-control tooltips" name="hc_date_end" value="${hc_date_ends }" autocomplete="off"   onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})" readonly="readonly" />
					                        </div>
					                        <div class="col-sm-1">
		                     ...
		                     <div class="form-group">
		                        ...
		                        <div class="col-sm-9">
		                            <textarea rows="4" name="disease" value="${chef.disease}" class="form-control"></textarea>
		                        </div>
		                    </div>
		                    <div class="form-group">
		                        <label class="col-sm-2  control-label">登记意见:</label>
		                        <div class="col-sm-9">
		                            <textarea rows="4" name="opinion" value="${chef.opinion }" class="form-control"></textarea>
		                        </div>
		                    </div>
		                    <div class="form-group">
		                        <label class="col-sm-2  control-label">管理记录:</label>
		                        <div class="col-sm-9">
		                            <textarea rows="4" name="mange_record" value="${chef.mange_record }" class="form-control"></textarea>
		                        </div>
		                    </div>
	...
	$('#doback').click(function(){ 
						    location.href = "<%=basePath%>chef/chefIndex?ac=<%=ac%>&searchName=${searchName}";
					 });
	var setting = {
					    check: {
							enable: true,
					...
					function onCheck(e, treeId, treeNode) {
						...
						var pnode ;
						var ppnode
					    pnode = zTree.getNodeByParam("id", nodes[0].pId, null);
						
						if(pnode==null){
						addName=nodes[0].name;
						addIds=nodes[0].id;
						}else{
						ppnode = zTree.getNodeByParam("id", pnode.pId, null); 
							if(ppnode==null){
							addName=pnode.name+nodes[0].name;
							addName2=pnode.name+","+nodes[0].name;
							addIds=pnode.id+","+nodes[0].id;
							}else{
								addName=ppnode.name+pnode.name+nodes[0].name;
								addName2=ppnode.name+","+pnode.name+","+nodes[0].name;
								addIds=ppnode.id+","+pnode.id+","+nodes[0].id;
							}
						}
						
						$("#orgSel1").attr("value", addName);
						$("#orgId1").attr("value", nodes[0].id);
						$("#address_village").attr("value", addIds);
						$("#address_town").attr("value", addName2);
					}
